...
public class carp extends AppCompatActivity implements AdapterView.OnItemSelectedListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        service_person = intent.getStringExtra("W2");
    ...
    private DatePickerDialog.OnDateSetListener dpickerlistner = new DatePickerDialog.OnDateSetListener() {
        ...
        public void onDateSet(DatePicker datePicker, int i, int i1, int i2) {
            ...
            day_x = i2;
            if (year_x != year_c)
                Toast.makeText(carp.this, "INVALID DATE", Toast.LENGTH_LONG).show();
                ...
                if (day_x <= day_c)
                    ...
                    Toast.makeText(carp.this, day_x + "/ " + month_x + "/ " + year_x, Toast.LENGTH_LONG).show();
    ...
    public void onClick(View v)
    {
        if(c1.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Chair";
            else
                services_required=services_required+", Chair";
        }
        if(c2.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Table";
            else
                services_required=services_required+", Table";
        }
        if(c3.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Lock";
            else
                services_required=services_required+", Lock";
        }
        if(c4.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Almirah";
            else
                services_required=services_required+", Almirah";
        }
        if(c5.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Bed";
            else
                services_required=services_required+", Bed";
        }
