...
public class elect extends AppCompatActivity implements AdapterView.OnItemSelectedListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        service_person = intent.getStringExtra("W3");
    ...
    private DatePickerDialog.OnDateSetListener dpickerlistner = new DatePickerDialog.OnDateSetListener() {
        ...
        public void onDateSet(DatePicker datePicker, int i, int i1, int i2) {
            ...
            day_x = i2;
            if (year_x != year_c)
                Toast.makeText(elect.this, "INVALID DATE", Toast.LENGTH_LONG).show();
                ...
                if (day_x <= day_c)
                    ...
                    Toast.makeText(elect.this, day_x + "/ " + month_x + "/ " + year_x, Toast.LENGTH_LONG).show();
    ...
    public void onClick(View v) {
        if(c1.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Fan";
            else
                services_required=services_required+", Fan";
        }
        if(c2.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Room-Light";
            else
                services_required=services_required+", Room-Light";
        }
        if(c3.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Switch";
            else
                services_required=services_required+", Switch";
        }
        if(c4.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Socket";
            else
                services_required=services_required+", Socket";
        }
        if(c5.isChecked())
        {
            if(services_required.equals("null"))
                services_required="Desk-Light";
            else
                services_required=services_required+", Desk-Light";
        }
